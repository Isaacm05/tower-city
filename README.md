...
Battery saver is enabled by default in **Settings → Performance**. The colony keeps
moving at 24 fps while interacting, 12 fps after ten seconds without input, and
10 fps when the window loses focus. Hidden tabs pause rendering and thread scans;
returning refreshes the roster immediately. High-density rendering is capped at
1.25 pixels per CSS pixel, without changing the saved quality preset. Turn off
Battery saver for up to 60 fps and your chosen render resolution.

## Adding projects

Choose **+ Add project** in the repo sidebar to select an existing workspace folder
(including nested folders), or create a new folder directly inside the workspace.
Added projects appear even before their first conversation and are remembered in
`data/projects.json` (or `BOT_CROSSING_DATA`). The workspace defaults to the parent
of the Bot Crossing checkout; set `BOT_CROSSING_WORKSPACE` to override it.

Optionally initialize Git or create a GitHub repository, private by default.
GitHub setup requires Git and the GitHub CLI (`gh auth login`), creates a repository
in the signed-in account, and connects `origin`. It does not commit or push files.
Existing Git repositories and origin remotes are preserved. If repository setup
fails after the folder is added, the project remains available and the dialog
explains the failure. Re-add the existing folder to retry setup.
