...
export class Colony {
   ...
   */
  setThreads(threads, archivedIds = new Set(), hiddenProjects = new Set(), knownIds = new Set(), addedProjects = []) {
    const now = Date.now()
    ...
    for (const project of addedProjects) {
      if (!hiddenProjects.has(project.name)) byProject.set(project.name, [])
    }
