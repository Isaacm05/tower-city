...
import { FrameLoop, frameRate } from './frame-loop.js'
...
export class Engine {
  constructor(settings) {
    ...
    this.frameLoop = new FrameLoop(this._boundLoop, () => this._frameRate())
    this._onInteraction = () => { this._lastInteraction = performance.now() }
  ...
  mount(parent) {
    ...
    // black flash.
    this._onWake = () => {
      if (document.hidden) { this.frameLoop.stop(); return }
      if (!this.running) return
      if (!this.frameLoop.running) this.timer.reset()
      this._onInteraction()
      this.resize()
      this.renderFrame()
    ...
    for (const event of ['pointerdown', 'pointermove', 'wheel', 'keydown']) {
      window.addEventListener(event, this._onInteraction, { passive: true })
    }
  ...
  _targetScale() {
    const requested = this.settings.get('renderScale') * (window.devicePixelRatio || 1)
    return this.settings.get('batterySaver') ? Math.min(requested, 1.25) : requested
  }

  _frameRate() {
    return frameRate({ hidden: document.hidden, focused: document.hasFocus(),
      saver: this.settings.get('batterySaver'), idle: performance.now() - this._lastInteraction > 10000 })
  }
  ...
  start() {
    ...
    this.running = true
    this.timer.reset()
    if (!document.hidden) this.frameLoop.start()
  }
  ...
  _loop() {
    ...
    const target = this._frameRate()
    if (target !== this._targetFps) {
      this._targetFps = target
      this._slow = this._fast = 0
      this._lastGovern = performance.now() + 2000
    }
    this.timer.update()
  ...
  _governQuality() {
    ...
    const target = this._targetFps || 60
    this._slow = fps < target * 0.75 ? (this._slow || 0) + 1 : 0
    this._fast = fps > target * 0.96 ? (this._fast || 0) + 1 : 0
