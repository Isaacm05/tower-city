...
let threads = []
let addedProjects = []
/** Who is actually installed on this machine — fetched once at boot; rarely changes mid-session. */
let harnesses = []
...
// A double-click or an impatient extra Enter while an open request is still in flight used to
// fire a second `/api/open` before the terminal it opens can be tagged for dedupe, landing two
// terminals for one thread. One in-flight request at a time closes that race.
...
const actions = {
  addProject: () => showAddProject(async (result) => {
    ...
    state.hiddenProjects = unhideProject(state.hiddenProjects || [], project.name)
    queueSave()
   ...
   *
   * Which harness answers is asked only when there is an actual choice: a machine with one
   * detected harness goes straight there, the same as before this had a picker at all. With
   * more than one, the project's own most-common harness is offered as the default rather than
   * silently chosen, since a repo touched from both is exactly the case a silent guess gets
   * wrong half the time.
   */
  newConversation: async () => {
    ...
    if (detected.length > 1) {
      const chosen = await hud.pickHarness(detected, { preferred: harness })
      if (!chosen) return
      harness = chosen
    }
    try {
  ...
  /**
   * The hud owns the textarea and the revision it last saw; this only knows which folder that
   * revision belongs to. A 409 (someone else — another tab, another session — saved first)
   * surfaces as a thrown error, same as any other failure; the hud's own catch shows it and
   * leaves the draft exactly as typed, matching what the server's own message promises.
   */
  ...
  /**
   * Adds a marketplace the user found and typed in — same registry `fetchSkillRegistry` reads,
   * just no longer limited to what was already in `extraKnownMarketplaces` when the server
   * started. Returns the fresh skill list straight off the response so the hud can repaint the
   * registry modal without a second fetch.
   */
  addMarketplace: async (source) => (await addMarketplace(source)).skills,

  /**
   * Installs into whichever project's sidebar is open, then forces the next `loadProjectExtras`
   * to actually refetch — the cooldown that keeps `setSkills` from re-running on every poll
   * would otherwise leave the freshly-installed skill invisible until the next natural refresh.
   */
  ...
  /** Same refetch-forcing shape as `installSkill`, the other direction. */
  ...
  /** Moves a trashed skill back into place — the way to get one like a hand-authored "eli5" back
   * after removing it, since unlike a marketplace install there's no other source to reinstall
   * it from. */
  ...
  // A PDF/doc goes to the server for `markitdown` to turn into markdown text. An image is
  // copied to the clipboard as an image instead of going through markitdown at all — Claude
  // Code already reads images natively, and markitdown's own image handling needs a paid Azure
  // endpoint (Document Intelligence / Content Understanding) to produce anything; without one
  // it silently returns empty text, which is why converting a screenshot used to paste as
  // nothing. Either way this is a one-shot "get it into a shape Claude Code can read", not
  // something the colony's own state needs to remember.
  ...
  copyImage: async (file) => {
    ...
    try {
      ...
      // Windows terminals (Wave Terminal included) often intercept Ctrl+V before Claude Code's
      // own paste handler sees it — Alt+V is the CLI's own Windows shortcut for pasting an
      // image, confirmed against Claude Code's own docs.
...
/**
 * Skills and the `.agent-context.md` for whichever project's sidebar is open. `syncProject`
 * calls this on every poll (every `POLL_MS`) and every selection change, so it is throttled to
 * that same cadence via `extrasFetchedAt` rather than fetching on every call — but it is a
 * cooldown, not a one-shot: a skill installed, or a context file edited, mid-session still shows
 * up on the next poll instead of needing a project switch to notice at all.
 *
 * A failed fetch does not start that cooldown — `extrasFetchedAt` only advances on success — so
 * the very next call (the next poll, at most `POLL_MS` later) retries on its own rather than
 * waiting for the project to be reselected.
 */
let extrasPath = null
...
async function loadProjectExtras(path) {
  if (path !== extrasPath) {
    extrasPath = path
    extrasFetchedAt = 0 // a genuinely new project always fetches immediately, cooldown or not
    // Cleared up front, not left showing the previous project's answer for the length of this
    // fetch — a stale answer mistaken for a fresh one is worse than a blank section.
    hud.setSkills([])
  ...
  extrasInFlight = (async () => {
    try {
      ...
      if (extrasPath !== forPath) return // a different project was picked while this was in flight
      hud.setSkills(skillsRes.skills || [], skillsRes.trashed || [])
      hud.setContext(contextRes)
      extrasFetchedAt = Date.now()
    } catch {
      // Left as whatever is already on screen; extrasFetchedAt stays put so the next poll retries.
    } finally {
      extrasInFlight = null
    }
  })()
...
function syncProject() {
  ...
  const projectPath = pathForProject(plot.name)
...
// ── paste & drag-drop: the same convert-to-markdown pipeline as the upload button ──────

window.addEventListener('paste', (e) => {
  // Only acts when the clipboard actually holds a file/image — plain text paste (into the
  // context editor, say) is left alone, since browsers do nothing with an image paste into a
  // text field anyway and every other paste target on this page is that kind of field.
...
// Entering a child element fires its own dragenter before the parent's dragleave — count
// depth rather than toggling on every leave, or the overlay flickers while dragging over
// the sidebar, the canvas, and back.
...
function applyThreads(list) {
  ...
  const stats = colony.setThreads(list, archivedSet, hiddenSet, known, addedProjects)
  hud.setStats(stats)
...
async function boot() {
  ...
  const [, kitError, crewError] = await Promise.all([
    ...
    fetchHarnesses()
      ...
      .catch(() => {
        /* the new-conversation picker just falls back to whichever harness a project already uses */
      }),
    settle(loadKit()),
    settle(loadCrew()),
  ])
...
engine.add({
  update(dt, elapsed) {
    ...
    if (settings.get('sound') && ambience.ctx?.state === 'running') {
...
/** The async clipboard needs a permission this page does not always have — inside an embedded
 * preview, say — so it falls back to the old selection-based copy, which has no such gate. */
...
/** Base64 payload only — `FileReader`'s data URL prefix (`data:...;base64,`) is stripped. */
...
/**
 * The Clipboard API only reliably accepts `image/png` in a `ClipboardItem` — writing a JPEG or
 * WebP blob under its own MIME type is rejected in Chromium. Already-PNG files pass through
 * untouched (a `File` is already a `Blob`); anything else is redrawn onto a canvas and
 * re-encoded.
 */
