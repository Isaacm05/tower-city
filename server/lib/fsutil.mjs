...
/**
 * A symlink or Windows junction reports as neither a file nor a directory in the raw dirent
 * `readdir` hands back — that flag reflects the link itself, not what it points at, so a plain
 * `e.isDirectory()` filter silently drops every linked directory. Only symlink entries pay for
 * the extra `stat` to find out what they actually resolve to; everything else is unaffected.
 */
