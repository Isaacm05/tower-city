...
async function latestStateDatabase() {
  ...
  return legacy || modernStateDatabase()
}

/**
 * Newer Codex CLI builds (0.154.x observed) moved the state db to `CODEX_HOME/sqlite/*.db` —
 * `codex-dev.db` on this machine — with no version number in the filename to sequence by, so
 * mtime is all there is. Only reached when no legacy `state_N.sqlite` exists.
 */
async function modernStateDatabase() {
  const dir = path.join(CODEX_HOME, 'sqlite')
  let entries
  try {
  ...
  for (const e of entries) {
    ...
    try {
      candidates.push({ file: path.join(dir, e.name), mtime: (await fsp.stat(path.join(dir, e.name))).mtimeMs })
    } catch {
...
async function scanThreads() {
  ...
  for (const id of ids) {
    ...
    const lifecycleType = facts.lifecycle?.type
    const running = lifecycleType === 'task_started' && now - lastActivityAt < ACTIVE_WINDOW_MS
    const hasError = lifecycleType === 'task_complete' && facts.lifecycle.error
    // `task_complete`/`turn_aborted` both hand the turn back to you — the same "last assistant
    // message didn't call a tool" signal `claude-code.mjs`'s `awaitingReply` reads off its own
    // transcripts, just told directly by Codex's own lifecycle events instead of inferred.
    const unread = !running && !hasError && (lifecycleType === 'task_complete' || lifecycleType === 'turn_aborted')
    ...
    out.push({
      ...
      // `unread` above stands in for the part that matters: whether the ball is in your court.
      lastFocusedAt: 0,
      unread,
      running,
      hasError,
      ...
      canOpen: true,
      // `cwd` rides along the same way claude-code.mjs's ref does: `codex resume` needs the
      // folder the session ran in, not just its id.
...
/**
 * Where the `codex` CLI is, for a machine that has it but nothing registered for `codex://` —
 * same reasoning as `claude-code.mjs`'s `cliBinary`: Linux and Windows both ask, since neither is
 * guaranteed to have the desktop app or VS Code extension installed; macOS trusts the deep link,
 * so the walk is wasted there.
 */
const CLI_DIRS = [
  path.join(HOME, '.local', 'bin'),
  '/usr/local/bin',
  '/usr/bin',
  // npm's default global-bin directory on Windows.
  path.join(process.env.APPDATA || path.join(HOME, 'AppData', 'Roaming'), 'npm'),
]
const CLI_NAMES = process.platform === 'win32' ? ['codex.cmd', 'codex.exe'] : ['codex']
...
/**
 * `codex://` is registered by the Codex desktop app or VS Code extension; the OS opener handles
 * it when one of those is installed. Where neither is — the CLI-only case this machine is in —
 * `command` gives `server/api.mjs` a `codex resume <id>` to fall back to, the same shape
 * `claude-code.mjs` offers for its own dead-link case.
 */
async function openThread(ref) {
  ...
  if (process.platform === 'linux' || process.platform === 'win32') {
    ...
    // `id` (prefixed, like the colony's own thread id) is what `server/lib/win.mjs` dedupes an
    // already-open terminal block against.
    if (bin) command = { argv: [bin, 'resume', id], cwd: typeof ref?.cwd === 'string' ? ref.cwd : '', id: ID(id) }
  }
...
async function newSession(dir) {
  const url = `codex://threads/new?${new URLSearchParams({ path: dir })}`
  let command
  if (process.platform === 'linux' || process.platform === 'win32') {
    const bin = await cliBinary()
