 ...
 * Linux and Windows both ask, since neither is guaranteed to have the desktop app installed;
 * on macOS the deep link is always answered, so the walk is wasted there.
 */
const CLI_DIRS = [
...
/**
 * `findExecutable` matches a file by its exact name, and on Windows the CLI is never literally
 * a file named `claude` — npm's installer writes a POSIX shim by that name alongside the real
 * entry points, `claude.cmd` and `claude.ps1`, which is what a plain `child_process.spawn` can
 * actually run. `.cmd` first: Node auto-wraps it through `cmd.exe` without a shell, while
 * `.ps1` needs `powershell -File` and buys nothing a `.cmd` doesn't already cover here.
 */
...
async function openThread(ref) {
  ...
  let command
  if ((process.platform === 'linux' || process.platform === 'win32') && isCliId(cliSessionId)) {
    const bin = await cliBinary()
    // `id` is the same string this thread is keyed on everywhere else in the colony — the one
    // stable handle `server/lib/win.mjs` has to dedupe an already-open terminal block against.
    if (bin) {
      command = { argv: [bin, '--resume', cliSessionId], cwd: typeof cwd === 'string' ? cwd : '', id: ID(cliSessionId) }
    }
  }
