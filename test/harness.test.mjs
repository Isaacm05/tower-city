...
test('findExecutable refuses junk, and refuses a directory that sits on PATH', async () => {
  // Isolated from the real PATH: a host can have a malformed entry (a full file path instead of
  // a directory) that would otherwise make '.' resolve straight to that file and pass X_OK.
